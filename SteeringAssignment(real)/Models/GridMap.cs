...
namespace SteeringAssignment_real.Mangers
{
    public class GridMap
    {
        ...
        public void Draw()
        {
            for (int y = 0; y < _grid.GetLength(0); y++)
            {
                for (int x = 0; x < _grid.GetLength(1); x++)
                {
                    ...
                    {
                        Vector2 start = new(_grid[y, x].X, _grid[y, x].Y + _map.TileSize.Y / 2);
                        Vector2 end = new(_grid[y, x + 1].X, _grid[y, x + 1].Y + _map.TileSize.Y / 2);
                    ...
                    {
                        Vector2 start = new(_grid[y, x].X + _map.TileSize.X / 2, _grid[y, x].Y);
                        Vector2 end = new(_grid[y + 1, x].X + _map.TileSize.X / 2, _grid[y + 1, x].Y);
        ...
        public Point GetNearestGridPoint(Vector2 position)
        {
            ...
            for (int row = 0; row < _grid.GetLength(0); row++)
            {
                for (int column = 0; column < _grid.GetLength(1); column++)
                {
                    ...
                    Vector2 gridPoint = new(_grid[row, column].X + _map.TileSize.X / 2, _grid[row, column].Y + _map.TileSize.Y / 2);
        ...
        public List<Point> GetNeighbouringPoints(Vector2 position)
        {
            ...
            List<Point> neighbouringPoints = new();
